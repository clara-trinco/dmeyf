...
-   ¿Es posible encontrar ganancia positiva en un corte sobre una variable?
    CT: No, Como seteamos la funcion de ganancia ganancias todos los que perdiamos los pusimos en 0, y todos los que tienen positivos quedan del otro lado.
    Si ponemos Baja +2 tenemos muchos negativos y no podemos distribuir en todos tantas bajas 2 para generar ganancia positivas.
=======
-   ¿Es posible encontrar ganancia positiva en ambos intervalos en un corte sobre una variable?

>>>>>>> 905b883109ecf1e3a5d8d166c934acb1b34a0214
-   ¿Cuál es la cardinalidad de los *intervalos* que dan ganancia?
    CT: Si tenemos muca ganancia positiva, el intervalo donde tengo mas ganancia son muy chicos. Porque tengo pocos baja + 2 pero su ganancia es mucho mejor.
    
-   ¿Cómo trata los árboles las variables con datos ausentes?
    CT: Los maneja primero divide por los que tiene y a os NA los clasifica en la clase que le parece mas relevante (mas entropia, gini) o lo deja en una tercera. 

-   Usualmente se suele cortar las variables en 2 intervalos
    
    -   ¿Se podría cortar en más intervalos?
    CT: si se podria, usamos regularmente arboles binarios pero podriamos definir mas cortes.
    
    -   ¿Cuál sería el costo?
    CT: mayor complejidad de computos
    
    -   ¿Se obtendrían mejores resultados?
    CT: posiblemente con arboles mas pequeños.
    
-   ¿Cuanto le afecta el """desbalanceo""" de datos al algoritmo?
    CT: El desbalanceo no afecta al arbòl. 
...
QUe pasa si tomo todas las ganancias positivas, porque en esas ramas tenemos 1/40 de relaciones de baja +2, entonces para cada hojas nos da dos cosas:
el target (y) y una probalidad de 0,025 a los que tienen ese valor seran clasificados como baja mas 2.
No miramos la clase ni la hoja, sino la proporcion. Si es inferior a 1/40 ganamos plata, por eso buscamos eso. 

Ganancia: define cuanto perderiamos/ganariamos si clasificamos como baja +2.
...
********************
********************
CT: sobre esto puedo calcular la matriz de confuncion con distintos umbrales: las probabilidaddes de salida (p-evento):
Por ejemplo con 0,036
TP: (4+2+3+197)/(238k)
FN:(0+1+3+5225)/(238k)
FP: (183+138+49+199)/(238k)
TN: (12095+10479+13697+193079)/(238K)
********************
********************
...
Cuanto error estoy puedo aceptar cuanto voy a dar positivos. La relacion de cuanto arriesgo con cuanto gano.
...
CT: Global, calcula todos los puntos de corte
CT: local seria, nos interesa los poquitos casos de BAJA+2. Ganancia es una metrica local por ejemplo.

-   ¿Pueden dos curvas distintas tener un mismo valor de **AUC**?
CT: si podemos tener mas de una 
...
hojasordenadas[, rec:= (( (1-b^2) * ( (tp/(tp+fp)) * (tp/(tp+fn)) ) / ( (b^2 * (tp/(tp+fp)) ) + (tp/(tp+fn)) ) )) ]

ggplot(hojasordenadas, aes(x=p_evento,y=rec)) + 
  geom_line(lwd=1) +
  geom_vline(xintercept = 0.025, linetype="dotted")
...
semillas<- c(999979,899971,799949,699863,599803)
```
...
ggplot(resultados_md[type == "test",], aes(x=ganancia)) + geom_histogram()
```
...
CT 1; la validation solo se usa 1 vez. 
