...
karch_dataset  <-  "./datasets/dataset_stacking_v0004.csv.gz"
ksalida  <- "semillerio_stacking_7010i1" 

kcantidad_semillas  <- 30

#ATENCION
#aqui deben ir los mejores valores que salieron de la optimizacion bayesiana
x  <- list()
x$gleaf_size   <-  29
x$gnum_leaves  <-  0.57533
x$learning_rate <-  0.122223
x$feature_fraction <-  0.282184
x$num_iterations  <- 237
...
`%notin%` <- Negate(`%in%`) # REGEX creado para eliminar meses malos
dataset <- dataset[foto_mes%notin% c(202003,202004,202005,202006)] #sacamos meses malos que vimos que sacandolos (meses extraños COVID) da mejores resultados.
...
for( semilla in  ksemillas)
{
  ...
  if(  isemilla %% 5 == 0 )  #imprimo cada 5 semillas
  {
    ...
    #------------ #Intento de generar modelitos-----------------------------
    
    #Genero modelito
    entrega_modelito  <- as.data.table( list( "numero_de_cliente"= dfuturo $numero_de_cliente, "Predicted"= tb_predicciones$prediccion)  )
    
    #Grabo salida modelitos
    fwrite( entrega_modelito,
            paste0( "./modelitos/modelitos_1430_stacking7010i1", ksalida , ".csv.gz" ),
            logical01 = TRUE,
            sep= "," )
